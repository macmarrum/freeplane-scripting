...
c.selecteds.each { n ->
    ...
    units.each { unit ->
        if (unit.size() > 2 && SummaryNode.isFirstGroupNode(unit[0].delegate)) {
            ...
            internals.sort { Node it -> it.lastModifiedAt }.reverse(true)
            unit.clear()
            unit << first
            unit.addAll(internals)
            unit << last
        } else {
            unit.sort(true) { Node it -> it.lastModifiedAt }.reverse(true)
        }
    }
    ...
    units.sort(true) { List<Node> unit ->
        if (unit.size() > 1 && SummaryNode.isFirstGroupNode(unit[0].delegate)) {
            ...
            body*.lastModifiedAt.max()
        } else {
            unit[0].lastModifiedAt
        }
    }.reverse(true) as LinkedList<List<Node>>
